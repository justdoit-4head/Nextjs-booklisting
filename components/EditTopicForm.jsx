...
export default function EditTopicForm({ id, title, author, publishYear }) {
  const [newTitle, setNewTitle] = useState(title);
  const [newAuthor, setnewAuthor] = useState(author);
  const [newpPublishYear, setNewpPublishYear] = useState(publishYear);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/api/books/${id}`, {
        ...
        body: JSON.stringify({ title: newTitle, author: newAuthor , publishYear: newpPublishYear }),
      });
  ...
  return (
          <div className="w-full h-full flex justify-center">
            ...
            <div className="mb-10">
          <h1 className="text-3xl font-extrabold leading-none tracking-tight text-gray-900 dark:text-blue-600 sm:mb-0 xs:mb-0">Edit your book</h1>
        </div>
      ...
      />
              ...
              <div class="relative z-0 w-full mb-5 group">
              <input
        onChange={(e) => setNewpPublishYear(e.target.value)}
        value={newpPublishYear}
        type="text"
        name="publish_year"
        id="publish_year"
        className="block lg:py-3 lg:text-lg px-0 w-full text-lg text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-black dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
